...
pub fn get_zed_cli_path() -> Result<PathBuf> {
    ...
    let possible_locations: &[&str] = if cfg!(target_os = "macos") {
        ...
        // bin/edna.exe is for installed builds, ./cli.exe is for development builds.
        &["bin/edna.exe", "./cli.exe"]
...
pub async fn load_login_shell_environment() -> Result<()> {
    ...
    {
        // Skip SHLVL to prevent it from polluting Edna's process environment.
        // The login shell used for env capture increments SHLVL, and if we propagate it,
        // terminals spawned by Edna will inherit it and increment again, causing SHLVL
